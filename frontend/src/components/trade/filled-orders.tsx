"use client"

import { useFilledOrders } from "../../hooks/useCLOB"

export function FilledOrdersPanel() {
  const { filled, loading, error } = useFilledOrders(4000)

  return (
    <div className="rounded-lg border bg-card p-6 h-full flex flex-col">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-lg font-semibold">Filled Orders</h3>
        {loading && <span className="text-xs text-muted-foreground animate-pulse">Updating...</span>}
      </div>
      {error && (
        <div className="text-xs text-destructive mb-2">Failed to load filled orders</div>
      )}
      <div className="grid grid-cols-4 text-xs font-medium text-muted-foreground pb-2 border-b">
        <div>Time</div>
        <div className="text-right">Price</div>
        <div className="text-right">Amount</div>
        <div className="text-right">Taker</div>
      </div>
      <div className="mt-2 space-y-1 overflow-y-auto max-h-72 pr-1">
        {filled.length === 0 && !loading && (
          <div className="text-xs text-muted-foreground text-center py-6">No fills yet</div>
        )}
        {filled.map((f, i) => (
          <div key={i} className="grid grid-cols-4 text-sm items-center rounded hover:bg-muted/50 p-1">
            <div className="font-mono text-xs">{new Date(f.timestamp).toLocaleTimeString()}</div>
            <div className={`text-right font-mono ${f.takerSide === 'buy' ? 'text-green-500' : 'text-destructive'}`}>{parseFloat(f.price).toFixed(2)}</div>
            <div className="text-right font-mono">{parseFloat(f.amount).toFixed(4)}</div>
            <div className={`text-right text-xs ${f.takerSide === 'buy' ? 'text-green-500' : 'text-destructive'}`}>{f.takerSide.toUpperCase()}</div>
          </div>
        ))}
      </div>
      
    </div>
  )
}
